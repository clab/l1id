...
gflags.DEFINE_string("corrected_dir", "",
    "Corrected with spellchecker files root directory")

gflags.DEFINE_string("restored_function_words_dir", "",
    "Hidden-ngram predicted files root directory")
...
class RestoredFunctionWordsFeatureExtractor(feature_extractor.FeatureExtractor):
  def __init__(self, corrected_dir, restored_function_words_dir):
    self.corrected_dir = corrected_dir
    self.restored_function_words_dir = restored_function_words_dir
  ...
  def ExtractFeaturesFromInstance(self, text, language, filename):
    ...
    corrected_filename = os.path.join(self.corrected_dir, os.path.basename(filename))
    predicted_filename = os.path.join(self.restored_function_words_dir, os.path.basename(filename))
...
def REGISTER_FEATURE_EXTRACTOR():
  if not FLAGS.append_restored_function_words_features:
    return None
  if len(FLAGS.restored_function_words_dir) == 0:
    print "Flag --restored_function_words_dir is required"
    sys.exit(1)
  return RestoredFunctionWordsFeatureExtractor(FLAGS.corrected_dir, FLAGS.restored_function_words_dir)
