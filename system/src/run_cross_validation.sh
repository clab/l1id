...
for ((i = 0; i < NUM_FOLDS; i++)); do
  ...
  ${BIN_DIR}/insert-first.pl '## FRE CHI ARA TEL JPN GER HIN ITA TUR KOR SPA' ${TRAIN_LABEL}
  ${BIN_DIR}/insert-first.pl '## FRE CHI ARA TEL JPN GER HIN ITA TUR KOR SPA' ${TEST_LABEL}
  ...
  ${CREG_BIN} -x ${TRAIN_FEAT} -y ${TRAIN_LABEL} --l2 1.0 --tx ${TEST_FEAT} \
      --ty ${TEST_LABEL} -D --z ${OUT_WEIGHTS} ${CMD_PREV_WEIGHTS} \
