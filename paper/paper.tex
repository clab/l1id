...
{\large Chris Dyer$^1$ Manaal Faruqui$^1$ Noam Ordan$^2$ Nathan Schneider$^1$\\ Yulia Tsvetkov$^1$ Naama Twitto$^2$ Shuly Wintner$^2$}\\[-3em]
}

\author{
\\
$^1$Language Technologies Institute\\Carnegie Mellon University\\Pittsburgh, PA\\\texttt{cdyer@cs.cmu.edu}
\And
\\
$^2$Department of Computer Science\\University of Haifa\\Haifa, Israel\\\texttt{shuly@cs.haifa.ac.il}
%
%
...
our methodology in Section~\ref{sec:methodology}. As in other author
...
Section~\ref{sec:features}. We report our results in
Section~\ref{sec:results} and conclude with suggestions for future
research.
...
Section~\ref{sec:literature}, many are novel, and are inspired by the
