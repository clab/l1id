...
the feature's value.  For each feature type, we report the accuracy of using just that feature type in
isolation on the training set\cjd{training? development? cross validated?} in Tab.~\ref{tbl:mainfeats}.


\begin{compactdesc}
\item[\feat{POS}] Part-of-speech $n$-grams.  Features were extracted
  to count every POS 1-, 2-, 3- and 4-gram in each
  document. 55.18\%. \ignore{CHI:IN PRP VBD, ARA:NN VBZ, KOR:DT JJ} 
\item[\feat{FreqChar}] Frequent character $n$-grams.  We experimented
  ...
  than $5$ times in the training corpus, and $n$ ranges from~1
    to~4. Accuracy: $74.12\%$. High-weight features include: {\tt TUR:Turk}, {\tt ITA:Ital}, {\tt JPN:Japa}.\item[\feat{CharPrompt}] Conjunction of the character $n$-gram
  features defined above with the prompt ID. Accuracy: 65.09\%.
\item[\feat{Brown}] Substitutions, deletions and insertions counts of Brown cluster unigrams and bigrams in each document. Accuracy: 72.26\%. \ignore{in top-30 features Brown features occur up to 5 times in 10 out of 11 L1s (all except Arabic)} 
\end{compactdesc}
\noindent
