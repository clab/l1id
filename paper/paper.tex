...
Training and development set released by the NLI Shared task \citep{tetreault-EtAl:2012:PAPERS} is a balanced set of 1000 TOEFL essays per L1,  10 to 20 sentences each, 900 documents for training and 100 for development. Each document is annotated with a writers English proficiency level (low, medium, high) and a prompt (1 to 8). All essays are tokenized and split to sentences. We detail in  \Tref{tbl:stats:level} training corpora statistics split by writers proficiency.
...
For classification we use the {\tt creg} regression modeling framework to train a 11-class logistic regression classifier.\footnote{https://github.com/redpony/creg}  We parameterize the L1 classifier as a multiclass logistic regression:
\begin{align*}
